...
export const api = {
  ...
  getProgressUrl(repoId: string): string {
    return `${BASE_URL}/repositories/${repoId}/progress`;
  },

  async getRepositoryStatus(repoId: string): Promise<{ repository_id: string; status: string; message?: string }> {
    const res = await fetchApi(`${BASE_URL}/repositories/${repoId}/status`);
  ...
  async getRepositories(): Promise<Repository[]> {
    const res = await fetchApi(`${BASE_URL}/graph/repositories`);
    if (!res.ok) throw new Error('Failed to load repositories');
    return res.json();
  },

  async analyzeRepository(url: string): Promise<{ repository_id: string; status: string }> {
    const res = await fetchApi(`${BASE_URL}/repositories/analyze`, {
      method: 'POST',
  ...
  async deleteRepository(repoId: string): Promise<{ status: string; message: string }> {
    const res = await fetchApi(`${BASE_URL}/repositories/${repoId}`, {
      method: 'DELETE'
    });
    if (!res.ok) {
  ...
  async getRepositoryGraph(repoId: string): Promise<RepositoryGraphResponse> {
    const res = await fetchApi(`${BASE_URL}/graph/repositories/${repoId}/data`);
  ...
  async expandNode(repoId: string, nodeId: string): Promise<RepositoryGraphResponse> {
    const res = await fetchApi(`${BASE_URL}/graph/repositories/${repoId}/expand/${encodeURIComponent(nodeId)}`);
    if (!res.ok) throw new Error('Failed to expand node');
    return res.json();
  },

  async getNodeDetails(nodeId: string, repoId: string): Promise<NodeDetails> {
    const res = await fetchApi(`${BASE_URL}/graph/nodes/${encodeURIComponent(nodeId)}?repo_id=${encodeURIComponent(repoId)}`);
    if (!res.ok) throw new Error('Failed to get node details');
    return res.json();
  },

  async traverseNode(nodeId: string): Promise<RepositoryGraphResponse> {
    const res = await fetchApi(`${BASE_URL}/graph/nodes/${encodeURIComponent(nodeId)}/traversal`);
  ...
  async createChatSession(repoId: string): Promise<ChatSession> {
    const res = await fetchApi(`${BASE_URL}/chat/sessions`, {
      method: 'POST',
  ...
  async getChatSession(sessionId: string): Promise<ChatSession> {
    const res = await fetchApi(`${BASE_URL}/chat/sessions/${sessionId}`);
  ...
  async sendChatMessage(
    ...
    const res = await fetchApi(`${BASE_URL}/chat/sessions/${sessionId}/messages`, {
      method: 'POST',
